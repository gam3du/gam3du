...
pub(super) struct SurfaceWrapper {
...
impl SurfaceWrapper {
    ...
    pub(super) fn resize(&mut self, context: &GraphicsContext, size: PhysicalSize<u32>) {
    ...
    pub(super) fn acquire(&mut self, context: &GraphicsContext) -> wgpu::SurfaceTexture {
        let surface = self.surface.as_ref().unwrap();
    ...
    pub(super) fn get(&self) -> Option<&wgpu::Surface<'_>> {
        self.surface.as_ref()
    }
