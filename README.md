...
🇩🇪 Hier findest du eine kurze [Deutschsprachige Einführung](#deutschsprachige-einführung)
...
If you asked Joe Average how many computers he posesses, he likely answer something between "none" to maybe "two". Reminding Joe that everything with a display is a computer as well, he might come up with some additional smartphones, smartwatches, tablets and TVs.
...
This _someone_ is a software developer - often casually called a _programmer_. As developers don't grow on [trees](https://en.wikipedia.org/wiki/Tree_(data_structure)), we need to resort to a more classic approach: _education_. While this sound like a downer at first, most good developers I (@kawogi) know, are enthusiastic about tech-stuff in general: give them something interesting to tinker with and they'll give you something impressive back.

Nowadays schools are starting to teach programming classes from the 5th grade at the age of 10 (at least in Germany). One great resource is [Scratch](https://scratch.mit.edu/). Scratch lets you build little games and animations by visually arranging command blocks. Another platforms like [Jugendwettbewerb Informatik](https://jwinf.de/) provide a series of programming challenges of increasing difficulties which can be solved by either using the visual Scratch-approach or using the Python programming language. This is a nice transition towards a textual programming language, which is the industry norm.
...
(a little upfront disclaimer for the list below: make sure to not break the law or harm someone else; multiplayer-cheating can be illegal)

- Level 1: Play a game. Find rules and patterns. Recognize similarities and differences to other games. Play with the configuration to get more frames per second out of it.
- Level 2: Exploit the game. Find loop-holes, glitches, cheats, try to push the boundaries of the engine, make the game crash.
- Level 3: Go backstage. Have a look at the game files. Maybe change some textures, swap audio files, have a look into configuration files and play with them.
- Level 4: Learn that you should have created a backup _before_ entering Level 3!
- Level 5: Modify the game. Find out whether the game allows to be _modded_. Download existing mods and have a look at the code. Try to change something. Read into the game's [API](https://en.wikipedia.org/wiki/API) to see whether you can find something of interest. Try to break something and GOTO 4 on success.
- Level 6: Write a mod from scratch. Congratulations, you've just become a developer!
- Level 7: Find the game's source code (if it is open source) and try to recognize what's happening inside of the _black box_. Maybe learn the language it's written in, write shaders, extend the API, exfiltrate statistics from the game, fix a bug, …
- Level 8: Write a new game using an existing game engine. Congratulations, you've just become a game developer!
- End boss: write your own game engine so that others can start at Level 1.

## What we do

We're fighting the end boss: Writing our own game engine.

There are numerous game engines out there, so why write another one?
...
- _approachable_ - it shall always be easy to "just start coding". No complicated installation procedures, system dependencies, build-scripts, package managers, …
...
- Game programming with scripts: Students may use the platform to create simple games in a 3D-environment.
- Creative: creating 3D-models, sounds, textures, shader programming …
...
At the moment our small team is entirely located in Germany. Using English as the primary communication language isn't as widely accepted as in other (smaller) countries. We'll try to make up for this by providing localized front-ends for the entry-level programming tasks.
...
(short German introduction for students)

Schön, dass du zu uns gefunden hast! _Gam3du_ ist eine Plattform auf der man spielerisch programmieren lernen kann.

Wenn alles gut läuft, dann kann man bald einfache Spiele (auch in 3D!) selbst erstellen.

Momentan ist noch vieles im Aufbau, aber du kannst gerne schonmal deinen Informatiklehrer oder deine Informatiklehrerin auf diese Seite aufmerksam machen. Vielleicht will er oder sie uns irgendwie unterstützen.
