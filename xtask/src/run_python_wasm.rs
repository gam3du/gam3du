...
pub(crate) fn run_wasm(shell: &Shell, mut args: Arguments) -> anyhow::Result<()> {
    ...
    xshell::cmd!(
        shell,
        "cargo build --target wasm32-unknown-unknown --package runtime-python-wasm {release_flag...}"
    )
    .args(&cargo_args)
    .quiet()
    .run()
    ...
    xshell::cmd!(
        shell,
        "wasm-bindgen target/wasm32-unknown-unknown/{output_dir}/runtime_python_wasm.wasm --target web --no-typescript --out-dir target/generated --out-name runtime_python_wasm"
    )
    .quiet()
    .run()
